    ...
    <x-slot name="header">
        ...
        <div>
            <div class="max-w-7xl ml-2 mx-auto sm:px-6>
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <table class="flex justify-center my-3.5 w-full">
                        ...
                        <th class="p-1 pr-14"><a href="{{ route('FilialBranchMailAllIncoming', ['surname'])}}">Отправитель</a></th>
                        ...
                        @foreach ($messages as $item)
                            <tr class="hover:bg-gray-100">
                                ...
                                <td class="p-1 pr-14">
                                    <a href="{{ route('FilialBranchMailRead1', [$item->id]) }}">{{$item->topic}}</a>
                                </td>
                                <td class="p-1 pr-1 text-xs">
                                    {{$item->created_at}}
                                </td>
                                <th class="p-1 pr-1">
                                    <x-link.delete delete="{{ route('FilialBranchMailDeleteUser1', [$item->id]) }}" />
                                </th>
            ...
            <div class="flex justify-center">
                ...
                <button form="selectedopt"  formaction="{{ route('FilialBranchMailDeleteMultiUser1')}}" type="submit" formmethod="get" class="bg-green-200 rounded m-3 p-3 hover:bg-green-400 ">Удалить</button>
                <br>
            </div>
        </div>
